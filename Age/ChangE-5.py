...
def calculate_age_CE5(craters_df, region_bounds, area_km2):
    """使用嫦娥5号模型计算年龄"""
    ...
    # 使用嫦娥5号修正模型: N(1,t) = 1.089×10⁻¹³(e⁶·⁷⁵⁷ᵗ - 1) + 7.660×10⁻⁴t
    try:
        # 使用数值方法求解t（因为方程非线性且t在两处出现）
        def equation(t):
            return 1.089e-13 * (np.exp(6.757*t) - 1) + 7.660e-4 * t - N1
        ...
        for _ in range(100):  # 最多迭代100次
            ...
            if abs(f) < 1e-10:  # 收敛条件
                break
            df = 1.089e-13 * 6.757 * np.exp(6.757*t) + 7.660e-4  # 导数
            t = t - f/df
            if t < 0:  # 确保年龄为正
                t = 0.1
...
for i, region in enumerate(regions):
    x1, y1, x2, y2 = region
    age = calculate_age_CE5(craters_df, region, area_per_region)
...
# 计算整个表面的年龄
whole_surface_age = calculate_age_CE5(craters_df, (0, 0, 4000, 4000), 200 * 200)
results.append({
    'Region': 5,
...
results_df.to_csv('Age\\4\\4_CE5.csv', index=False)
print("Results saved to 2_CE5.csv")
